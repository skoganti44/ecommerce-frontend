...
export default function BakeryDashboard() {
  ...
  const load = () => {
    ...
    fetchBakeryInStoreOrders()
      ...
      .catch((err) =>
        setError(err.message || 'Failed to load bakery counter queue.')
      )
    ...
    <Box sx={{ maxWidth: 1000, mx: 'auto' }}>
      <Paper elevation={0} sx={GLASS_PAPER}>
        <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 1 }}>
          <BakeryDiningIcon sx={{ color: '#ef5350', fontSize: 34 }} />
          <Typography
            variant="h5"
            sx={{
              fontWeight: 800,
          ...
          >
            Welcome to Bakery, {user?.name || 'Baker'}
          </Typography>
        </Stack>
        <Typography sx={{ mb: 2, color: '#6d4c41', fontWeight: 600 }}>
          Walk-in orders the kitchen has marked DONE — ready to hand to the
          customer at the counter.
        </Typography>

        <Stack direction="row" spacing={2} sx={{ mb: 2, flexWrap: 'wrap' }}>
          <Chip
            label={`${orders.length} order${orders.length === 1 ? '' : 's'} at counter`}
            sx={{ fontWeight: 700, bgcolor: '#ede7f6', color: '#4527a0' }}
          />
          <Chip
            label={`${totalItems} item${totalItems === 1 ? '' : 's'} to hand over`}
        ...
        {!loading && !error && orders.length === 0 && (
          <Alert severity="info">
            Nothing at the counter yet. Once the kitchen marks a walk-in order
            DONE, it'll show up here.
          </Alert>
        )}
